...
| Field | Meaning |
| --- | --- |
| `AM` | Assistant Manager shared improvement family |
| `IMP` | Improvement item |
| `YYYY.MMDD` | Date the reusable improvement is defined |
| `NN` | Sequence number on that day |

## Improvement Lifecycle

| Status | Meaning |
| --- | --- |
| `Proposed` | Idea exists but no implementation spec yet. |
| `Ready` | Spec and upgrade checklist are clear enough to apply. |
| `Installed` | Applied to a project and verified locally. |
| `Deployed` | Applied and verified in production, such as Render. |
| `Blocked` | Cannot be applied until a dependency is resolved. |
| `Deprecated` | No longer recommended. |

## Separation Rule

Reusable improvements may share:
...
Reusable improvements must not share:

- LINE channel tokens or secrets
- Notion tokens
- Notion data source IDs across different business entities
- Render service IDs or production environment values
- Customer messages, task records, report data, or automation logs
...
docs/
  ...
  project-improvement-manifest.md
  templates/
    upgrade-version-template.md
  upgrades/
    UPGRADE-2026-06-08-AM-IMP-2026.0608.01.md
```
