// HOZO AM 三層下鑽 Dashboard（server-rendered, Basic auth 保護）：
...
export async function renderDashboardOverview() {
  ...
  return pageShell('HOZO AM Dashboard', body);
}
...
export async function renderProjectPage(projectName) {
  ...
  return pageShell(`${project.name} - HOZO AM`, body);
}
...
export async function renderTaskPage(taskPageId) {
  const tasksDataSourceId = requiredEnv('HOZO_TASKS_DATA_SOURCE_ID');
  ...
  (function () {
    ...
    button.addEventListener('click', async () => {
      const payload = { pageId: panel.dataset.page, editedBy: '陸昱晴' };
  ...
  (function () {
    ...
    document.getElementById('planned-send').addEventListener('click', async () => {
      ...
      const payload = { pageId: panel.dataset.page, editedBy: '陸昱晴', message };
  ...
  return pageShell(`${task.title} - HOZO AM`, body);
}
  ...
  try {
    ...
    for (const block of result.results || []) {
      ...
      if (meta) {
        ...
        current = { meta: line.slice(0, 80), text: '', outgoing: /HOZO Jr\.|附件解析/.test(line), images: [], files: [], links: [] };
        continue;
      }
      if (current) {
...
async function queryAllProjects() {
  const projectsDataSourceId = process.env.HOZO_PROJECTS_DATA_SOURCE_ID || '6395278e-53e8-4b47-917a-36d88802324e';
  try {
