...
const controlLinePushUrl = process.env.CONTROL_LINE_PUSH_URL || 'https://hozo-am-line-oa-webhook.onrender.com/control/line/push';
...
async function proposeProjects() {
  return llm.completeJson({
    maxTokens: 8000,
    system: [
      '你是 HOZO AM 控制中心的專案治理分析師。你的工作：從零散訊號中辨識「正在形成但還沒有專案容器的工作流」，提出專案候選。',
      '',
      '## 原則',
...
async function notifyProposals(items) {
  ...
  try {
    ...
    await fetch(controlLinePushUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json; charset=utf-8', 'x-hozo-control-key': controlApiKey },
