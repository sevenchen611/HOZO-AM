...
if (!casesDataSourceId) fail('HOZO_JUDGMENT_CALIBRATION_CASES_DATA_SOURCE_ID is not set.');
if (!rulesDataSourceId) fail('HOZO_JUDGMENT_RULES_DATA_SOURCE_ID is not set.');
...
async function createFeedbackCase(task) {
  ...
  const reviewId = `HOZO-FB-${formatDateKey(new Date())}-${task.id.slice(0, 6)}`;
      ...
      const rulePage = await notionRequest('/v1/pages', {
        ...
        body: {
          ...
          properties: compactProperties({
            ...
            'Applies To': { multi_select: [{ name: 'HOZO_AM' }] },
...
async function suggestRule(anthropic, task) {
  const response = await anthropic.messages.create({
    ...
    system: [
      '你是 HOZO AM 任務判讀系統的校準分析師。AI 從 LINE 對話萃取了一個任務，但使用者退回了它（不是任務）。',
