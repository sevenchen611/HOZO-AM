...
if (!commandsDataSourceId) fail('HOZO_CODEX_COMMANDS_DATA_SOURCE_ID is not set.');
...
async function maybeReply(command, text) {
  ...
  if (!controlApiKey) {
    console.warn('HOZO_CONTROL_API_KEY is not set; instant reply skipped.');
    return;
  }
...
async function runTriage(command) {
  return anthropic.completeJson({
    maxTokens: 8000,
    system: [
          '你是 HOZO AM（HOZO Assistant Manager）的指令分流助理。使用者透過 LINE 對 HOZO Jr. 下達指令，這些指令進入佇列等待處理。',
