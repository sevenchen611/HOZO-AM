...
let resolvedParentPageId = normalizeId(args.parent || process.env.HOZO_RESPONSIBILITY_PARENT_PAGE_ID || parentPageId);
...
async function createResponsibilityOwnerNarrowingDatabases() {
  ...
  let groupOptionsDataSourceId = normalizeId(args.groups || process.env.HOZO_LINE_GROUP_OPTIONS_DATA_SOURCE_ID || '');
  if (!groupOptionsDataSourceId) {
    groupDatabase = await createDatabase({
      title: 'HOZO LINE 群組選項庫',
      dataSourceTitle: 'HOZO LINE 群組選項',
  ...
  const memberDatabase = await createDatabase({
    title: 'HOZO LINE 群組成員選項庫',
    dataSourceTitle: 'HOZO LINE 群組成員',
  ...
  return {
    ...
    databases: {
      responsibility: {
        ...
        renderEnvVar: `HOZO_RESPONSIBILITY_DATA_SOURCE_ID=${responsibilityDataSourceId}`,
      },
      groupOptions: {
        databaseId: groupDatabase?.id || null,
        dataSourceId: groupOptionsDataSourceId,
        url: groupDatabase?.url || null,
        renderEnvVar: `HOZO_LINE_GROUP_OPTIONS_DATA_SOURCE_ID=${groupOptionsDataSourceId}`,
      },
      groupMembers: {
        databaseId: memberDatabase.id,
        dataSourceId: groupMembersDataSourceId,
        url: memberDatabase.url,
        renderEnvVar: `HOZO_LINE_GROUP_MEMBERS_DATA_SOURCE_ID=${groupMembersDataSourceId}`,
      },
    },
    nextStep: 'Run seed with these HOZO_* data source IDs, then run npm run responsibility:sync -- --dry-run.',
  };
}
