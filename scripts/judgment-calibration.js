...
async function createCalibrationDatabases() {
  const existingRulesDataSourceId = process.env.HOZO_JUDGMENT_RULES_DATA_SOURCE_ID || '';
  const existingCasesDataSourceId = process.env.HOZO_JUDGMENT_CALIBRATION_CASES_DATA_SOURCE_ID || '';
  ...
  return {
    created,
    env: {
      HOZO_JUDGMENT_RULES_DATA_SOURCE_ID: rulesDataSourceId,
      HOZO_JUDGMENT_CALIBRATION_CASES_DATA_SOURCE_ID: casesDataSourceId,
      HOZO_JUDGMENT_REVIEW_TARGET_NAME_KEYWORD: defaultTargetName,
    },
  };
}

async function resolveParentPageId() {
  const explicit = normalizeId(args.parent || process.env.HOZO_JUDGMENT_PARENT_PAGE_ID || process.env.HOZO_DATA_SOURCE_PARENT_BLOCK_ID || '');
  if (explicit) return explicit;
  if (!tasksDataSourceId) fail('HOZO_TASKS_DATA_SOURCE_ID is required to infer a parent page.');
  ...
  fail('Unable to infer parent page. Set HOZO_JUDGMENT_PARENT_PAGE_ID or HOZO_DATA_SOURCE_PARENT_BLOCK_ID.');
}

async function resolveReviewTarget() {
  const targetName = String(args['target-name'] || defaultTargetName).trim();
  if (!targetName) fail('Missing --target-name or HOZO_JUDGMENT_REVIEW_TARGET_NAME_KEYWORD.');
...
async function createCalibrationCase(casesDataSourceId, task) {
  ...
  ].filter(Boolean).join('；') || '由 HOZO AM 任務庫篩選為需核對項目。';

  return createPage(casesDataSourceId, {
    'Review ID': titleProperty(reviewId),
    Project: selectProperty('HOZO_AM'),
...
async function recordControllerReply(calibration, { reviewId, direction, reason, rule, exception }) {
  ...
  if (rule) {
    const rulePage = await createPage(calibration.rulesDataSourceId, {
      ...
      'Applies To': multiSelectProperty(['HOZO_AM']),
...
async function requireCalibrationDatabases() {
  const casesDataSourceId = process.env.HOZO_JUDGMENT_CALIBRATION_CASES_DATA_SOURCE_ID || '';
...
async function requireLinePush() {
  if (!controlApiKey) fail('HOZO_CONTROL_API_KEY is not set.');
}
